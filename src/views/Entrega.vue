...
import { ref, onMounted } from 'vue' // Corrigido: onMounted importado aqui!
...
async function carregar() {
  ...
  try {
    ...
    const { data: estoqueData } = await supabase.from('estoque').select('epi_id, quantity') // Atenção ao nome da coluna de quantidade no seu banco se for quantity ou quantidade
    if (estoqueData) {
      estoqueMap.value = Object.fromEntries(estoqueData.map(e => [e.epi_id, e.quantidade || e.quantity || 0]))
    }
...
async function registrar() {
  ...
  // Baixa no estoque (Ajustado dinamicamente para atualizar a tabela do banco)
...
.error-msg { color: #dc2626; font-size: 0.875rem; margin-top: 10px; font-weight: 600; }
.success-msg { color: #166534; font-size: 0.875rem; margin-top: 10px; font-weight: 600; }
.checkbox-group { justify-content: center; }
.checkbox-label { display: flex; align-items: center; gap: 8px; font-size: 0.875rem; cursor: pointer; color: #334155; font-weight: 600; }
</style>